...
# Encoder
x = layers.Dense(128, activation="relu", kernel_regularizer=regularizers.l2(0.0001))(
    input_layer
)
...
for col in X_all.columns:
    if col in clip_params:
        X_all[col] = np.clip(
            X_all[col], clip_params[col]["lower"], clip_params[col]["upper"]
        )
...
for at in labels[labels != "BENIGN"].unique():
    m = mse[labels == at]
    stats.append({"type": at, "n": len(m), "mean": m.mean(), "med": np.median(m)})
...
for s in stats:
    ...
    print(
        f"{st} {s['type']:<27} {s['n']:>8,} {s['mean']:>10.4f} {s['med']:>10.4f} {r:>7.1f}x"
    )
