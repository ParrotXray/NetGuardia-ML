...
try:
    ...
    print(
        f"📊 訓練時性能: TPR={best_strategy['tpr']:.1%}, F1={best_strategy['f1']:.3f}"
    )
...
if current_dim != expected_dim:
    raise ValueError(
        f"❌ 維度不匹配！模型需要 {expected_dim} 維，資料有 {current_dim} 維"
    )
...
for name, thresh in sorted(test_thresholds.items(), key=lambda x: x[1]):
    ...
    print(
        f"{name:<20} {thresh:<12.6f} {tp:<8} {fp:<8} {fn:<8} {precision:<10.4f} {recall:<10.4f} {f1:<10.4f}"
    )
...
for attack_type in sorted(labels[labels != "BENIGN"].unique()):
    ...
    print(
        f"{status} {attack_type:<32} {total:<10,} {detected:<10,} {rate:<9.1%} {mean_score:<12.6f}"
    )
...
if len(X_anomaly) > 0:
    ...
    if mask_real_attack.sum() > 0:
        ...
        print(
            f"📊 攻擊分類準確率（排除 BENIGN）: {attack_acc:.4f} ({attack_correct:,}/{len(true_attack_labels):,})"
        )

        print(f"\n📋 攻擊分類詳細報告:")
        try:
            report = classification_report(
                true_attack_labels, pred_attack_labels, zero_division=0, digits=4
            )
            print(report)
        except:
            print("  (無法生成報告)")
...
if len(false_negatives) > 0:
    ...
    for attack_type in sorted(fn_labels.unique()):
        ...
        print(
            f"  {attack_type:<35} {fn_type_count:>6,} / {total_type:<6,} ({pct:>5.1f}%)"
        )
...
ax1.hist(
    ensemble_benign, bins=100, alpha=0.7, label="BENIGN", color="green", density=True
)
ax1.hist(
    ensemble_attack, bins=100, alpha=0.7, label="Attack", color="red", density=True
)
ax1.axvline(
    threshold,
    color="black",
...
colors = [
    "gold" if r["name"] == threshold_name else "steelblue" for r in threshold_results
]
...
sample_idx2 = np.random.choice(
    len(ensemble_score), min(5000, len(ensemble_score)), replace=False
)
colors_scatter2 = [
    "red" if l != "BENIGN" else "green" for l in labels.iloc[sample_idx2]
]
...
ax10.plot(
    recalls[current_idx], precisions[current_idx], "r*", markersize=15, label="Current"
)
    ...
    if mask_real_attack.sum() > 10:
        ...
        if len(unique_labels) <= 15:  # 只在類別不太多時顯示
            ...
            ax12.text(
                0.5,
                0.5,
                f"Too many classes ({len(unique_labels)})",
                ha="center",
                va="center",
            )
            ax12.set_title("MLP Classification Matrix", fontweight="bold")
